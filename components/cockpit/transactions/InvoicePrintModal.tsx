...
export function InvoicePrintModal({
  ...
  const handleEmailToNotary = () => {
    ...
    const body = [
      `Bonjour Maître,`,
      ``,
      `Veuillez trouver ci-dessous les coordonnées bancaires de ${settings.agency_name || "SASU Nell'Immo"} pour le règlement de nos honoraires de transaction, à prévoir lors de la réitération par acte authentique.`,
      ``,
      `— FACTURE N° ${invoiceNo} —`,
      ...
      `IBAN : ${settings.agency_rib_iban || 'Non renseigné (à configurer dans Cockpit > Paramètres)'}`,
