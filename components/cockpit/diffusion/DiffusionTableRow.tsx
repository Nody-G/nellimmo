...
import { ShieldCheck, AlertTriangle, AlertCircle, Globe } from 'lucide-react';
...
export function DiffusionTableRow({
  ...
  const channels: {
    key: PortalChannelKey;
    label: string;
    /** Logo officiel du canal (undefined = repli sur une icône générique). */
    brand?: BrandKey;
    active: boolean;
    activeColor: string;
  }[] = [
    ...
    <tr className="hover:bg-gray-50/80 transition-colors group">
      ...
      <td className="py-3.5 px-3 text-center">
        <button
          ...
          className={`px-2 py-0.5 rounded-full text-[10px] font-bold inline-flex items-center gap-1 border transition hover:opacity-80 cursor-pointer ${compliance.status === 'compliant'
      ...
      {channels.map((ch) => (
        <td key={ch.key} className="py-3.5 px-1.5 text-center">
          <button
            ...
            className={`inline-flex items-center justify-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold border transition cursor-pointer ${ch.active
              ? ch.activeColor
          ...
          >
            {ch.brand ? (
              <BrandLogo brand={ch.brand} size={12} className={ch.active ? '' : 'opacity-40 grayscale'} />
            ) : (
              <Globe className="w-3 h-3" />
            )}
            <span>{ch.active ? 'Diffusé' : 'Non'}</span>
          </button>
        </td>
      ))}
    </tr>
