...
import { CheckCircle2, RefreshCw, LogIn, ExternalLink } from 'lucide-react';
...
export function GoogleAccountStatusCard({
  ...
  const accountEmail = formData.google_account_email || formData.google_calendar_id || 'nellimmo.acte@gmail.com';
  const connectedDate = formData.google_connected_at
    ? new Date(formData.google_connected_at).toLocaleDateString('fr-FR', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
      })
    ...
    <div className="p-4 bg-gradient-to-r from-blue-50/60 via-indigo-50/30 to-white rounded-2xl border border-blue-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-[10px] text-gray-400">
            Dernière synchronisation réussie le {connectedDate} &bull; 7 services reliés
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button
          ...
          onClick={onQuickSync}
          disabled={isSyncing}
          className="px-3 py-2 bg-white hover:bg-blue-50 border border-blue-200 text-blue-900 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-2xs transition disabled:opacity-50 cursor-pointer"
