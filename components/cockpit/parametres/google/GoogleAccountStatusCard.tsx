...
export function GoogleAccountStatusCard({
  ...
  // Source de vérité = Google (route status), repli sur les paramètres locaux.
  const accountEmail = status.email || formData.google_account_email || formData.google_calendar_id || '';
  ...
  // Nombre de services réellement autorisés côté Google (pas un chiffre inventé).
  const enabledCount = Object.values(status.services).filter(Boolean).length;
        ...
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-gray-900">
              {status.name || formData.google_account_name || 'Compte Google Workspace de Nelly'}
            </span>
            {isLoading ? (
          ...
          <p className="text-[10px] text-gray-400">
            {isConnected
              ? `${enabledCount > 0 ? `${enabledCount} service(s) autorisé(s)` : 'Aucun service autorisé'}${connectedDate ? ` • depuis le ${connectedDate}` : ''}`
              : status.configured
                ? 'Connectez votre compte Google pour activer la synchronisation des services.'
                : 'Configuration OAuth serveur incomplète (.env.local).'}
          </p>
