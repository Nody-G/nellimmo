...
/** Section : Sauvegarde, Portabilité & Conformité RGPD */
export function BackupSection({ formData, showToast }: BackupSectionProps) {
  ...
  const handlePurge = () => {
    ...
    showToast(
      `Purge RGPD effectuée : ${result.purgedCount} fiche(s) supprimée(s) (${result.remainingCount} conservées).`,
      'success'
    );
  };
  ...
  return (
    <SectionCard
      ...
      title="Sauvegarde, Portabilité & Conformité RGPD"
    >
      <p className="text-xs text-gray-600">
        Téléchargez une copie intégrale de sécurité de toute votre activité (17 collections, registre ALUR scellé SHA-256) ou restaurez un fichier JSON.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
        ...
        >
          <div>
            <span className="font-bold text-xs text-gray-900 group-hover:text-emerald-800 block">
              Exporter Sauvegarde Master (JSON)
            </span>
            <span className="text-[11px] text-gray-500">Toutes les données en 1 clic</span>
          </div>
        ...
        <label className="p-4 bg-[#FCFAF7] hover:bg-amber-50 border border-[#F3E8EE] hover:border-amber-200 rounded-2xl flex items-center justify-between transition cursor-pointer text-left group">
          <div>
            ...
            <span className="text-[11px] text-gray-500">Charger sauvegarde précédente</span>
          </div>
        ...
        <Link
          ...
          className="p-4 bg-[#FCFAF7] hover:bg-purple-50 border border-[#F3E8EE] hover:border-purple-200 rounded-2xl flex items-center justify-between transition cursor-pointer text-left group"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-xs text-gray-900 group-hover:text-purple-900 block">
                Migration Hektor (CSV)
              </span>
              <span className="text-[11px] text-gray-500">Import catalogue & acquéreurs</span>
            </div>
          </div>
        ...
        {isConfirmingPurge ? (
          <div className="p-3 bg-red-50 border border-red-200 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-3 animate-fade-in">
            <span className="text-xs text-red-800 font-medium">
              Confirmer la suppression irréversible de {inactiveCount} fiche(s) ?
            </span>
