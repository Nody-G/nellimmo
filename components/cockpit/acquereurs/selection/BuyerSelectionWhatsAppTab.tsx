...
export function BuyerSelectionWhatsAppTab({
  ...
  return (
    <div className="flex-1 flex flex-col justify-between space-y-3">
      {onGenerateAi && (
        <div className="flex items-center justify-between bg-white p-2.5 rounded-xl border border-gray-100 shadow-2xs">
          <div className="text-[11px] text-gray-500">
            {aiSource ? (
              <span className="font-semibold text-emerald-700 flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-[#C59A45]" />
                Personnalisé à la plume de Nelly ({aiSource === 'deepseek' ? 'DeepSeek' : 'Local'})
              </span>
            ) : (
              'Personnalisez ce message selon les critères de cet acquéreur :'
            )}
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span>{isGeneratingAi ? 'Rédaction...' : '🪄 Plume de Nelly (IA)'}</span>
          </button>
        </div>
      )}
